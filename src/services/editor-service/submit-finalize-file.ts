...
export async function submitFinalize(
    ...
    logger.info(
        `--> OrderTranscriptionCFFlow:submitFinalize ${order.fileId}`,
    );
    try {
        ...
        if (!fileVersion?.s3VersionId) {
            logger.error(`OrderFlow:submitReview - Finalize docx file ${order.fileId} has not been uploaded yet for`);
            throw new Error(`OrderFlow:submitReview - Finalize docx file ${order.fileId} has not been uploaded yet`)
        }
        ...
        logger.error(
            `OrderTranscriptionCFFlow:submitFinalize ${order.id}-${order.fileId} ${(error as Error).toString()}`,
        );
        return {
            success: false,
