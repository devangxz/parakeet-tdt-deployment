  ...
  try {
    if (type === InvoiceType.TRANSCRIPT) {
      ...
      fileInfo.forEach((file, index) => {
        ...
        body += `<td style='text-align:left;border:1px solid #cccccc;padding:5px' align='left' border='1' cellpadding='5'><a target='_blank' href='https://${process.env.SERVER}/files/selected/${file.fileId}'>${file.File.filename}</a></td>`
      ...
      const templateData = {
        file_url: `https://${process.env.SERVER}/files/selected/${invoice.itemNumber}`,
