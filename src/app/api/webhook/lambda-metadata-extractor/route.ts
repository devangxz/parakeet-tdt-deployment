    ...
    try {
        ...
        if (metadata.status === 'SUCCESS') {
            ...
            try {
                ...
                logger.error(`Error creating audio/video conversion job - userId: ${metadata.userId}, fileId: ${metadata.fileId}, fileKey: ${metadata.fileKey}:`, error);
            }
        }
        ...
        logger.error(`Error processing lambda-metadata-extractor webhook for file ID ${metadata.fileId}:`, error);
        return NextResponse.json({ error: `Error processing lambda-metadata-extractor webhook for file ID ${metadata.fileId}` }, { status: 500 });
    }
}