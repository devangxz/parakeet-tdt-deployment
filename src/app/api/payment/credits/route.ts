...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, s: response })
  } catch (error) {
    logger.error(`Error fetching credits balance for ${selectedId}`, error)
    return NextResponse.json({
      success: false,
      message: 'Error fetching credits balance s.',
    })
  }
}
