    ...
    try {
        ...
        for (const file of assignedFiles) {
            if (file.order.File?.duration) {
                let durationInMs = file.order.File.duration * config.job_timeout_multiplier * 60 * 1000

                if (file.extensionRequested) {
                    durationInMs += file.order.File.duration * extensionTimeMultiplier * 60 * 1000
                }
        ...
        logger.info(`Timed out files have been rejected: ${rejectedFiles.join(', ')}`)
