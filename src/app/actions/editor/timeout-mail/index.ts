...
const sendTimeoutMail = async (
  ...
  try {
    ...
    logger.info(
      `Sending timeout mail to ${userEmailId} for order ${orderDetails.orderId}`
    )
    let durationInMs = calculateTimerDuration(fileData.duration)
    if (jobAssignment.extensionRequested) {
      durationInMs +=
        fileData.duration * config.extension_time_multiplier * 1000
    }
    const response = await ses.sendMail(
      'QC_JOB_TIMEOUT',
      {
        userEmailId,
      },
    ...
    logger.info(
      `Timeout Warning mail sent to ${userEmailId} for order ${
        orderDetails.orderId
      } ${JSON.stringify(response)}`
    )
  } catch (error) {
    logger.error(
      `Error sending timeout mail to ${userEmailId} for order ${orderDetails.orderId}: ${error}`
    )
  }
}
