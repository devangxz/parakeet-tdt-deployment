...
const LinkImporter: React.FC<UploaderProps> = ({ onUploadSuccess }) => {
  ...
  const initializeMultiPartUpload = async (
    ...
    const checkRes = await checkUploadSession(fileName, totalSize, 'link', url)
  ...
  const multiPartUpload = async (
    ...
    let state = await initializeMultiPartUpload(
      fileName,
      fileId,
      contentType,
      contentLength,
    ...
    try {
      ...
      while (true) {
        try {
          const downloadResponse = await fetch('/auth/link', {
            method: 'POST',
  ...
  const handleImport = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const file of initialFiles) {
        try {
          const headResponse = await axios.post('/auth/link', {
            url: file.url,
            method: 'HEAD',
          })
          ...
          if (!contentType || !contentLength) {
            toast.error(
              `Invalid file at URL: ${file.url} - Missing content type or size`
            )
            updateUploadStatus(file.name, {
              progress: 0,
              status: 'failed',
          ...
          if (contentLength > MAX_FILE_SIZE) {
            toast.error(
              `Invalid file at URL: ${file.url} - File exceeds 10GB size limit`
            )
            updateUploadStatus(file.name, {
              progress: 0,
              status: 'failed',
      ...
      for (const link of linksToUpload) {
        try {
          ...
          toast.error(
            `Import failed for ${link.fileName}. Please note that if you try importing the same file again after a few minutes, it will automatically resume from where it stopped.`
          )
        }
      }
    } catch (error) {
