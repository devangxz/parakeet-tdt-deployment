...
import { BACKEND_URL, DEFAULT_USER_OPTIONS } from '@/constants'
...
const Page = () => {
  ...
  const [orderOptions, setOrderOptions] =
    useState<orderOptoinsType>(defaultInitState)
  ...
  async function handleDefaultInstruction() {
    ...
    try {
      const response = await axiosInstance.post(
        `${BACKEND_URL}/default-instructions`,
        {
  ...
  useEffect(() => {
    axiosInstance
      .get(`${BACKEND_URL}/default-options`)
      .then((response) => {
        setOrderOptions({ ...orderOptions, ...response.data.options })
        if (response?.data?.instructions !== null)
          setDefaultInstruction(response.data.instructions.instructions)
      })
      .catch((error) => {
        if (error instanceof AxiosError && error.response) {
          const message = mapKeyToMessage(error.response.data.message)
          const errorToastId = toast.info(message)
          toast.dismiss(errorToastId)
        } else {
          toast.error(`Error fetching default settings ${error}`)
        }
