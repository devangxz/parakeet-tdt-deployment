...
const Page = () => {
  ...
  const [preferencesOptions, setPreferencesOptions] =
    useState<preferecesOptoinsType>(defaultInitState)
  ...
  async function handlePreferences() {
    ...
    try {
      ...
      const response = await axiosInstance.post(
        `${BACKEND_URL}/default-preferences`,
        {
  ...
  useEffect(() => {
    axiosInstance
      .get(`${BACKEND_URL}/default-preferences`)
      .then((response) => {
        setPreferencesOptions({
          ...preferencesOptions,
          ...response.data.preferences,
        })
        const { recordsPerPage } = response.data.recordsPerPage
      ...
      .catch((error) => {
        if (error instanceof AxiosError && error.response) {
          ...
          toast.error(`Error fetching mail notifications`)
        }
      })
      .finally(() => {})
  }, [])
