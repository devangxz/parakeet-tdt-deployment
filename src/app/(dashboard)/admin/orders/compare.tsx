...
export default function ComparePage() {
  ...
  const compareFiles = async () => {
    ...
    try {
      ...
      const res = await getCompareFiles(reviewDiff, verificationDiff, fileId)
      if (!res.success) {
        toast.error(res.message)
        return
      }
      const diff = diffWords(res.reviewFile ?? '', res.verificationFile ?? '')
      setDiff(diff)
  ...
  return (
    <div className='h-full flex-1 flex-col space-y-8 p-8 md:flex'>
      <div className='flex items-center justify-between space-y-2'>
        <div>
          <h1 className='text-lg font-semibold md:text-lg'>Compare Files</h1>
        </div>
      </div>
      <div>
        ...
        <div className='flex w-full justify-between mt-10'>
              ...
              <Select onValueChange={setReviewDiff} defaultValue={reviewDiff}>
                ...
                <SelectContent>
                  ...
                  {verificationDiff !== 'customer-delivered' && (
                    <SelectItem value='customer-delivered'>
                      Customer Delivered
                    </SelectItem>
                  )}
          ...
          <div className='border rounded-sm w-[45%] py-2 px-5'>
            ...
            <div className='my-5'>
              <Select
                onValueChange={setVerificationDiff}
                defaultValue={verificationDiff}
              >
                <SelectTrigger>
                  <SelectValue placeholder='Select version' />
                </SelectTrigger>
        ...
        <div className='flex justify-center items-end mt-10'>
          ...
          <Button className='ml-2' onClick={compareFiles}>
            Compare
          </Button>
        </div>
      </div>
