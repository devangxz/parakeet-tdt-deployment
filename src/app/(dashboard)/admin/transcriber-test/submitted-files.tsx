...
import { DataTable } from '@/app/(dashboard)/admin/orders/components/data-table'
import QCLink from '@/app/(dashboard)/admin/orders/components/qc-link'
import { getListenCountAndEditedSegmentAction } from '@/app/actions/admin/get-listen-count-and-edited-segment'
import { getSignedUrlAction } from '@/app/actions/get-signed-url'
import { fetchTestApprovalFiles } from '@/app/actions/om/fetch-test-approval-files'
import OpenDiffDialog from '@/components/admin-components/diff-dialog'
import PassFailTranscriberTestDialog from '@/components/admin-components/pass-fail-transcriber-test'
...
interface SubmittedFilesPageProps {
  onActionComplete?: () => Promise<void>
}

export default function SubmittedFilesPage({
  onActionComplete,
}: SubmittedFilesPageProps) {
  ...
  return (
    <>
      <div className='h-full flex-1 flex-col space-y-8 p-8 md:flex'>
        <div className='flex items-center justify-between space-y-2 mt-1'>
          <div>
            <h1 className='text-lg font-semibold md:text-lg'>
              Submitted Test Files ({approvalFiles?.length})
            </h1>
          </div>
        </div>
        <DataTable
