...
const UploadProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const updateUploadStatus = useCallback(
    (fileName: string, status: Partial<UploadStatus>) => {
  ...
  const initializeSSEConnection = useCallback(
    (onFileSuccess: () => void, onAllCompleted: () => void) => {
      ...
      try {
        eventSourceRef.current = new EventSource('/sse')
        ...
        eventSourceRef.current.onerror = () => {
          ...
          if (
            ...
            errorCountRef.current < SSE_MAX_RETRIES
          ) {
            setTimeout(() => {
              if (eventSourceRef.current) {
                closeSSEConnection()
