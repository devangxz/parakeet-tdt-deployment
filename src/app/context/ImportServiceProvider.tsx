...
interface ImportServiceState {
  ...
  dropboxInitError?: string
  boxInitError?: string
  oneDriveInitError?: string
  googleDriveInitError?: string
}
...
const ImportServiceContext = createContext<
  ImportServiceContextType | undefined
>(undefined)
...
export const useImportService = () => {
  ...
  if (context === undefined) {
    throw new Error(
      'useImportService must be used within an ImportServiceProvider'
    )
  }
  return context
}

const ImportServiceProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  ...
  const initializeDropbox = async () => {
    try {
      ...
      setState((prev) => ({ ...prev, dropboxInitError: errorMessage }))
    }
  }
    ...
    try {
      switch (service) {
        case 'isBoxServiceReady':
          await new Promise<void>((resolve, reject) => {
            document
              .querySelectorAll('script[src*="box.com"]')
        ...
        case 'isOneDriveServiceReady':
          await new Promise<void>((resolve, reject) => {
            document
              .querySelectorAll('script[src*="onedrive.js"]')
              .forEach((script) => script.remove())
        ...
        case 'isGoogleDriveServiceReady':
          await new Promise<void>((resolve, reject) => {
            document
              .querySelectorAll('script[src*="google"]')
              .forEach((script) => script.remove())
            ...
            Promise.all([
              ...
              window?.gapi?.load('picker', () => {
                if (window.google?.picker) {
                  setState((prev) => ({
                    ...prev,
                    isGoogleDriveServiceReady: true,
                  }))
                  resolve()
                } else {
      ...
      setState((prev) => ({
        ...prev,
        [service === 'isBoxServiceReady'
          ? 'boxInitError'
          : service === 'isOneDriveServiceReady'
          ? 'oneDriveInitError'
          : 'googleDriveInitError']: errorMessage,
      }))
    }
  }
