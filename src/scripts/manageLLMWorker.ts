...
const LLMQueue = new Queue('llm_marking', { connection: redis });

const RENDER_API_KEY = process.env.RENDER_API_KEY;
const RENDER_WORKER_SERVICE_ID = process.env.RENDER_LLM_WORKER_SERVICE_ID;

const RENDER_API_URL = `https://api.render.com/v1/services/${RENDER_WORKER_SERVICE_ID}/scale`;

async function manageWorker() {
    try {
        const jobCounts = await LLMQueue.getJobCounts('waiting', 'active');
