...
const ASRQueue = new Queue('automatic_speech_recognition', { connection: redis });

const RENDER_API_KEY = process.env.RENDER_API_KEY;
const RENDER_WORKER_SERVICE_ID = process.env.RENDER_ASR_WORKER_SERVICE_ID;

const RENDER_API_URL = `https://api.render.com/v1/services/${RENDER_WORKER_SERVICE_ID}/scale`;

async function manageWorker() {
    try {
        const jobCounts = await ASRQueue.getJobCounts('waiting', 'active');
