...
import DefaultShortcuts, { getAllShortcuts, setShortcut, ShortcutControls, useShortcuts } from '@/utils/editorAudioPlayerShortcuts'
import { adjustTimestamps, downloadBlankDocx, downloadMP3, getFrequentTermsHandler, handleSave, insertTimestampBlankAtCursorPosition, navigateAndPlayBlanks, playCurrentParagraphTimestamp, regenDocx, replaceTextHandler, searchAndSelect } from '@/utils/editorUtils'
...
export default function Header({
    ...
    const playNextBlankInstance = () => {
        const quill = quillRef?.current?.getEditor();
        if (!quill) return;
        return navigateAndPlayBlanks.bind(null, quill, audioPlayer.current, setDisableGoToWord, false)
    }

    const playPreviousBlankInstance = useCallback(() => {
        const quill = quillRef?.current?.getEditor();
        if (!quill) return;
        return navigateAndPlayBlanks.bind(null, quill, audioPlayer.current, setDisableGoToWord, true)
    }, [audioPlayer, quillRef])

    const playCurrentParagraphInstance = useCallback(() => {
        const quill = quillRef?.current?.getEditor();
        if (!quill) return;
        return playCurrentParagraphTimestamp.bind(null, quill, audioPlayer.current, setDisableGoToWord)
    }, [audioPlayer, quillRef])
    ...
    const fetchWaveform = async () => {
        try {
            const res = await axiosInstance.get(`${FILE_CACHE_URL}/get-waveform/${orderDetails.fileId}`, { responseType: 'blob' })
    ...
    const replaceTextInstance = (findText: string, replaceText: string, replaceAll = false) => {
        if (!quillRef?.current) return;
        const quill = quillRef.current.getEditor();
        replaceTextHandler(quill, findText, replaceText, replaceAll, matchCase, toast);
    };

    const searchAndSelectInstance = (searchText: string) => {
        if (!quillRef?.current) return;
        const quill = quillRef.current.getEditor();
        searchAndSelect(quill, searchText, matchCase, lastSearchIndex, setLastSearchIndex, toast);
    };
    ...
    }, [findAndReplaceOpen, findText, replaceText, playNextBlankInstance,
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            if (!audioPlayer || !audioPlayer.current || !videoRef.current) return;
            if (videoRef.current && audioPlayer.current.currentTime !== videoRef.current.currentTime) {
    ...
    const insertTimestampBlankAtCursorPositionInstance = useCallback(() => {
        insertTimestampBlankAtCursorPosition(audioPlayer.current, quillRef?.current?.getEditor());
    }, [audioPlayer, quillRef]);
                    ...
                    <div className='w-full flex justify-between'>
                        <div className='flex'>
                            <TooltipProvider>
                                ...
                                <Tooltip>
                                    <TooltipTrigger>
                                        <PlayerButton icon={<ThickArrowRightIcon />} onClick={playNextBlankInstance()} tooltip='Play next blank' />
                                    </TooltipTrigger>
                                    <TooltipContent>
                                        <p>Play next blank</p>
                                    </TooltipContent>
                                ...
                                <Tooltip>
                                    <TooltipTrigger>
                                        <PlayerButton icon={<ThickArrowLeftIcon />} onClick={playPreviousBlankInstance()} tooltip='Play previous blank' />
                                    </TooltipTrigger>
                                    <TooltipContent>
                                ...
                                <Tooltip>
                                    <TooltipTrigger>
                                        <PlayerButton icon={<TextAlignLeftIcon />} tooltip='Play audio from the start of current paragraph' onClick={playCurrentParagraphInstance()} />
                                    </TooltipTrigger>
                                    <TooltipContent>
                                    ...
                                    <TooltipTrigger>
                                        <Dialog>
                                            <DialogTrigger asChild>
                                                <PlayerButton icon={<TimerIcon />} onClick={setSelectionHandler} tooltip='Adjust timestamps' />
                                            </DialogTrigger>
                                            <DialogContent>
                                                <DialogHeader>
                                ...
                                <Tooltip>
                                    <TooltipTrigger>
                                        <PlayerButton icon={<ZoomInIcon />} tooltip='Increase font size' onClick={increaseFontSize} />
                                    </TooltipTrigger>
                                    <TooltipContent>
                                ...
                                <Tooltip>
                                    <TooltipTrigger>
                                        <PlayerButton icon={<ZoomOutIcon />} tooltip='Decrease font size' onClick={decreaseFontSize} />
                                    </TooltipTrigger>
                                    <TooltipContent>
                                ...
                                <Tooltip>
                                    <TooltipTrigger>
                                        <PlayerButton icon={<MagnifyingGlassIcon />} tooltip='Find and replace' onClick={toggleFindAndReplace} />
                                    </TooltipTrigger>
                                    <TooltipContent>
                                ...
                                <Tooltip>
                                    <TooltipTrigger>
                                        <PlayerButton icon={<ClockIcon />} tooltip='Insert Timestamps' onClick={insertTimestampBlankAtCursorPositionInstance} />
                                    </TooltipTrigger>
                                    <TooltipContent>
