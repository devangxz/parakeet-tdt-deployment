...
interface ShortcutsReferenceDialogProps {
  ...
  setIsShortcutsReferenceModalOpen: React.Dispatch<
...
export const formatShortcutKey = (shortcut: string) =>
  shortcut
    .split('+')
    .map((word) => word.trim())
    .map((word) => {
...
export const formatAction = (key: string) =>
  key
    .replace(/([A-Z])/g, ' $1')
    .replace(/(\d+)/g, ' $1')
    .toLowerCase()
...
const ShortcutsReferenceDialog = ({
  isShortcutsReferenceModalOpen,
  setIsShortcutsReferenceModalOpen,
  shortcuts,
  setShortcuts,
}: ShortcutsReferenceDialogProps) => {
  ...
  return (
    <Dialog
      open={isShortcutsReferenceModalOpen}
      onOpenChange={setIsShortcutsReferenceModalOpen}
    >
      <DialogContent className='p-0 gap-0'>
        ...
        <div className='flex justify-end items-center gap-x-2 m-4 mt-1'>
          ...
          <Button
            onClick={() => setIsShortcutsReferenceModalOpen(false)}
            variant='outline'
          >
            Close
          </Button>
