...
> {
  try {
    ...
    const allRetryCounts = JSON.parse(
      (await redis.get('ASR_RETRY_COUNT')) || '{}'
    )

    if (!allRetryCounts[fileId]) {
      await redis.set(
        'ASR_RETRY_COUNT',
        JSON.stringify({ [fileId]: 1 }),
        'EX',
        3600
      )
    ...
    if (retryCount >= 3) {
      ...
      if (errorMessage.toLowerCase().includes('account balance is negative')) {
        ...
        await ses.sendAlert(
          ...
          `ASR failed for ${fileId} due to account balance being negative. Please add more credits to the account as soon as possible.`,
          'software'
        )
      }
      ...
      await redis.set(
        'ASR_RETRY_COUNT',
        JSON.stringify({ ...allRetryCounts, [fileId]: retryCount + 1 }),
        'EX',
        3600
      )
      await performASR(fileId)
